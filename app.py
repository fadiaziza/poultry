...
def search_engine(query, top_k=5):
    ...
    if alarms:
        for a in alarms:
            ...
            if m_num:
                ...
                matched = [p for p in manual_pages if re.search(pattern, p["text"], re.IGNORECASE)]
                if matched:
                    if any(k in clean_q for k in ["?????", "automac", "fabbri"]):
    ...
    # 3. ????? ??????? ?????????? ??????? ?????? ??????
    keywords_map = {
        "???????": (["maestro", "eviscerat"], ["infeed", "entry", "positioning", "shackle", "drawing", "guide"]),
        "?????": (["automac", "wrapping", "297", "298"], ["tray", "film", "alarm", "infeed", "stop"]),
        "?????": (["compressor", "chiller", "refrigeration"], ["temperature", "pressure", "oil", "cooling"]),
        "???????": (["compressor", "airpol", "atlas"], ["pressure", "filter", "separator", "alarm"]),
        "?????": (["plucker", "picking"], ["finger", "belt", "motor"]),
        "???": (["scalder", "scalding"], ["temperature", "water", "circulation"])
    }
...
def maintenance_copilot(query, input_image=None):
    ...
    if input_image is not None:
        ...
        if matched_part_no:
            response.append(f"?? **?? ?????? ?????? ??? ???? ??????:** `{matched_part_no}`")
    ...
    if hits:
        response.append(f"### ? ?? ?????? ??? ????? ?????? ?? ??????????:")
        for h in hits:
            response.append(f"- **?????:** `{h['filename']}` (???? {h['page']})")
   ...
   # ????? ???????? ???????? (???? ?? ?????? ????? ?????? ??????? ???????)
    ...
    if hits:
        alert_msg += f"?? ?????? ?????: {hits[0]['filename']} (???? {hits[0]['page']})\n"
    if matched_image_path:
        alert_msg += f"??? ??????: ?? ??????? ???? ?????? ?? ????? ????????."
...
logo_html = f'<img src="data:image/png;base64,{logo_base64}" style="width: 100%; height: 100%; object-fit: contain;">' if logo_base64 else '<span style="font-size: 20px; font-weight: 900; color: #1b5e20;">?????</span>'
...
<div style="background: linear-gradient(135deg, #0b3d20 0%, #1b5e20 100%); padding: 18px 25px; border-radius: 14px; color: white; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.18); direction: rtl; text-align: right; border-bottom: 4px solid #ffcc00;">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
        <div style="display: flex; align-items: center; gap: 20px;">
            ...
            <div>
                <h1 style="margin: 0; font-size: 23px; font-weight: 800; color: #ffffff;">???? ????? ?????? - ???? ?????</h1>
                <p style="margin: 4px 0 0 0; font-size: 14px; color: #e8f5e9;">???? ??????? ???????? ??????? ?????? (???? Meyn • ??????? ??????? Automac • ??????? ???????)</p>
            </div>
        </div>
        <div style="border-right: 2px solid rgba(255,255,255,0.25); padding-right: 20px;">
            <span style="font-size: 12px; color: #c8e6c9; display: block;">????? ?????? ??????:</span>
            <span style="font-size: 16px; font-weight: bold; color: #ffeb3b;">?. ???? ?????</span>
            <span style="font-size: 12px; color: #e8f5e9; display: block;">????? ??? ??????? ????????</span>
        </div>
    </div>
</div>
"""
...
with gr.Blocks(title="???? ??????? ???????? - ???? ?????") as demo:
    ...
    with gr.Row():
        status_box = gr.Markdown(f"?? **???? ??????:** ?? ????? ?????? `{total_manuals}` ?????? ??? ??????? ??? ??? ???????? ????????.")
        
    with gr.Row():
        with gr.Column(scale=1):
            query_input = gr.Textbox(
                label="???? ??? ??????? / ??? ?????? (4 ?????) / ??? ?????",
                placeholder="?????: ????? E002 ?????? ??????? | ????? ?????? ????????? | 0990.AD05.007.00 | 89 3608 904 0096",
                lines=2
            )
        ...
        with gr.Column(scale=1):
            output_box = gr.Markdown(label="????? ????? ????? ???????")
            matched_img_output = gr.Image(type="filepath", label="???? ?????? ???????? ?? ????? ????????")
